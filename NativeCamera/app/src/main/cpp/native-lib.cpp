...
Java_com_keiko_nativecamera_CameraCalib_calibCamera(JNIEnv *env, jobject instance,
   ...
   Mat imageCopy;
    ...
    Ptr<aruco::GridBoard> gridboard =GridBoard::create(markersX, markersY, markerLength, markerSeparation, dictionary);
   ...
   // putText(imageCopy, "Nevim co tady dat",Point(10, 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 0, 0), 2);
    if(ids.size() > 0) {
        ...
        putText(imageCopy,"Frame captured",Point(10, 20),FONT_HERSHEY_SIMPLEX,0.5,Scalar(255,0,128),2);
    }
    if(allIds.size() < 1) {
        cerr << "Not enough captures for calibration" << endl;
        __android_log_print(ANDROID_LOG_ERROR, "allIds.size() < 1", "Not enough captures for calibration");
        return;
    }
    ...
    if(!saveOk) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "Cannot save output file", "Cannot save output file");
